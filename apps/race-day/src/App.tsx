...
import { loadRaceConfiguration, loadRaceSession, saveRaceConfiguration, saveRaceSession, saveRunnerPreferences } from './configuration-repository';
...
import { AUDIO_CUES, PHASE_SWIPE_DISTANCE_PX, PHASE_SWIPE_VELOCITY, RACE_CLOCK_TICK_MS, SUPPRESS_TAP_AFTER_SWIPE_MS } from './race-constants';
import type { CSSProperties } from 'react';
import type { EffortLevel, MusicCue, NotificationSoundSettings, Phase, RaceConfiguration, RaceSettingsDraft, ViewMode } from './types';
...
export default function App() {
  ...
  const [race, dispatchRace] = useReducer((session: ReturnType<typeof loadRaceSession>, action: Parameters<typeof raceSessionReducer>[1]) => raceSessionReducer(session, action, planRef.current), undefined, loadRaceSession);
  ...
  useEffect(() => { planRef.current = plan; saveRaceConfiguration(configuration); }, [configuration, plan]);
  useEffect(() => { saveRunnerPreferences({ ...configuration.preferences, viewMode }); }, [configuration.preferences, viewMode]);
  useEffect(() => { void loadImportedMusicTracks().then(imported => setLibraryTracks(tracks => [...tracks, ...imported])).catch(() => undefined); }, []);
  useEffect(() => { raceRef.current = race; saveRaceSession(race); }, [race]);

  useEffect(() => {
    void synchronizeNativeRaceNotifications(configuration.plan, race, configuration.preferences.notificationSounds, configuration.preferences.notificationSoundVolumes).catch(() => undefined);
  }, [configuration.plan, configuration.preferences.notificationSounds, configuration.preferences.notificationSoundVolumes, race]);

  const { audioRef, beep, duration: musicDuration, isPlaying: musicPlaying, pause: stopMusic, play: startMusic, position: musicPosition, seek: seekMusic, unlockAudio } = music;
  ...
  useEffect(() => { const id = window.setInterval(() => setCurrentTime(Date.now()), RACE_CLOCK_TICK_MS); return () => window.clearInterval(id); }, []);
  useRaceCues({ planRef, sessionRef: raceRef, dispatch: dispatchRace, onInterval: mode => { const cue = mode === 'RUN' ? AUDIO_CUES.run : AUDIO_CUES.walk; beep(cue.frequency, cue.duration); }, onGel: cueGel });
  ...
  const togglePause = () => {
    ...
    if (race.pausedAt) { unlockAudio(); dispatchRace({ type: 'RESUME', now: Date.now() }); }
  ...
  const openSettings = () => { setDraft({ ...activePlan, notificationSounds: { ...activePlan.notificationSounds }, notificationSoundVolumes: { ...activePlan.notificationSoundVolumes }, phases: plan.map(phase => ({ ...phase })) }); setSettingsOpen(true); };
  const saveSettings = () => {
    if (!draft) return;
    setConfiguration({ plan: { presetId: draft.presetId, phases: draft.phases }, preferences: { notificationSounds: draft.notificationSounds, notificationSoundVolumes: draft.notificationSoundVolumes, musicVolume: draft.musicVolume, viewMode } });
  ...
  return <main className={`race ${viewMode === 'simple' ? 'race-simple' : 'race-full'}`} style={style}
    ...
    <header className="top"><span className="brand">NYC · RACE DAY</span><span className="top-actions"><button className="view-button" onClick={() => { const nextView = viewMode === 'simple' ? 'full' : 'simple'; setViewMode(nextView); setConfiguration(current => ({ ...current, preferences: { ...current.preferences, viewMode: nextView } })); }}>{viewMode === 'simple' ? 'Full controls' : 'Now view'}</button>{viewMode === 'full' && <button className="icon-button" aria-label="Open settings" onClick={openSettings}>⚙</button>}</span></header>
    {viewMode === 'simple'
      ? <SimpleRaceScreen phase={phase} phaseLabel={preset.isTest ? 'TEST CONFIG' : `PHASE ${race.phase + 1} / ${plan.length}`} intervalMode={interval.mode} intervalLeft={formatTime(interval.left)} intervalProgress={interval.progress} phaseProgress={phaseProgress} cycleLabel={`CYCLE ${Math.min(projection.effectivePlannedCycles, cycles.done + 1)} OF ${projection.effectivePlannedCycles}`} gelLeft={formatTime(gel.left)} paused={Boolean(race.pausedAt)} showMusic={musicEnabled} trackLabel={musicTrack.label} musicPlaying={musicPlaying} musicPosition={musicPosition} musicDuration={musicDuration} onToggleMusic={toggleMusic} onSeekMusic={seekMusic} />
      : <><section className="body" {...bindPhaseSwipe()}><span className="phase-count">{preset.isTest ? 'TEST CONFIG' : `PHASE ${race.phase + 1} / ${plan.length}`} · {theme.label}</span><h1>{phase.name}</h1><span className="miles">{phase.miles}</span><p className="note">{phase.note}</p>
        <div className="interval"><span className="mode">{interval.mode}</span><strong>{formatTime(interval.left)}</strong>{race.pausedAt && <span className="paused">PAUSED</span>}<ProgressIndicators intervalProgress={interval.progress} phaseProgress={phaseProgress} cycleLabel={`CYCLE ${Math.min(projection.effectivePlannedCycles, cycles.done + 1)} OF ${projection.effectivePlannedCycles}`} /><button className="gel-action" disabled={Boolean(race.pausedAt)} onClick={event => { event.stopPropagation(); logGelEarly(); }}><i /> GEL IN <b>{formatTime(gel.left)}</b><small>LOG EARLY</small></button>{musicEnabled && <MusicPlayer label={musicTrack.label} isPlaying={musicPlaying} position={musicPosition} duration={musicDuration} onToggle={toggleMusic} onSeek={seekMusic} />}<div className="cue-controls"><button onClick={event => { event.stopPropagation(); jumpCue(-1); }}>↶ Previous interval</button><button onClick={event => { event.stopPropagation(); jumpCue(1); }}>Next interval ↷</button><button onClick={event => { event.stopPropagation(); addCycle(); }}>＋ Add cycle</button></div><span className="tap-hint">Tap to {race.pausedAt ? 'resume' : 'pause'} · swipe left/right to change phase</span></div>
      </section>
    ...
    {settingsOpen && draft && <Settings activePlan={draft} libraryTracks={libraryTracks} setPlan={setDraft} onAddMusic={addMusicTrack} onClose={() => setSettingsOpen(false)} onSave={saveSettings} onLoadPreset={id => { const next = configurationFromPreset(id); const nextDraft: RaceSettingsDraft = { ...next.plan, ...next.preferences }; setDraft(nextDraft); setConfiguration(next); planRef.current = next.plan.phases; dispatchRace({ type: 'REPLACE_PLAN', phaseCount: next.plan.phases.length, now: Date.now(), begun: false }); stopMusic(); }} onReset={() => { const next = configurationFromPreset(draft.presetId); setDraft({ ...next.plan, ...next.preferences }); }} />}
  </main>;
}

function Settings({ activePlan, libraryTracks, setPlan, onAddMusic, onClose, onSave, onLoadPreset, onReset }: { activePlan: RaceSettingsDraft; libraryTracks: typeof musicTracks; setPlan: (plan: RaceSettingsDraft) => void; onAddMusic: (file: File) => Promise<void>; onClose: () => void; onSave: () => void; onLoadPreset: (id: string) => void; onReset: () => void }) {
  const preset = getPreset(activePlan.presetId);
  ...
  const updateSoundVolume = (cue: keyof NotificationSoundSettings, volume: RaceSettingsDraft['notificationSoundVolumes'][keyof RaceSettingsDraft['notificationSoundVolumes']]) => setPlan({ ...activePlan, notificationSoundVolumes: { ...activePlan.notificationSoundVolumes, [cue]: volume } });
  const previewSound = (soundId: string, volume: RaceSettingsDraft['notificationSoundVolumes'][keyof RaceSettingsDraft['notificationSoundVolumes']]) => { const audio = new Audio(`./${notificationSoundSource(soundId, volume)}`); audio.volume = 0.8; void audio.play().catch(() => undefined); };
...
function SoundSelect({ value, volume, onChange, onVolumeChange, onPreview }: { value: string; volume: RaceSettingsDraft['notificationSoundVolumes'][keyof RaceSettingsDraft['notificationSoundVolumes']]; onChange: (soundId: string) => void; onVolumeChange: (volume: RaceSettingsDraft['notificationSoundVolumes'][keyof RaceSettingsDraft['notificationSoundVolumes']]) => void; onPreview: (soundId: string, volume: RaceSettingsDraft['notificationSoundVolumes'][keyof RaceSettingsDraft['notificationSoundVolumes']]) => void }) { return <span className="sound-select"><select value={value} onChange={event => onChange(event.target.value)}>{notificationSounds.map(sound => <option key={sound.id} value={sound.id}>{sound.label}</option>)}</select><select value={volume} onChange={event => onVolumeChange(event.target.value as RaceSettingsDraft['notificationSoundVolumes'][keyof RaceSettingsDraft['notificationSoundVolumes']])}>{notificationSoundVolumes.map(level => <option key={level.id} value={level.id}>{level.label}</option>)}</select><button type="button" onClick={() => onPreview(value, volume)}>Preview</button></span>; }
...
function SimpleRaceScreen({ phase, phaseLabel, intervalMode, intervalLeft, intervalProgress, phaseProgress, cycleLabel, gelLeft, paused, showMusic, trackLabel, musicPlaying, musicPosition, musicDuration, onToggleMusic, onSeekMusic }: { phase: Phase; phaseLabel: string; intervalMode: string; intervalLeft: string; intervalProgress: number; phaseProgress: number; cycleLabel: string; gelLeft: string; paused: boolean; showMusic: boolean; trackLabel: string; musicPlaying: boolean; musicPosition: number; musicDuration: number; onToggleMusic: () => void; onSeekMusic: (position: number) => void }) {
  return <section className="simple-race-screen"><span className="phase-count">{phaseLabel}</span><h1>{phase.name}</h1><span className="miles">{phase.miles}</span><div className="simple-interval"><span>{paused ? 'PAUSED' : intervalMode}</span><strong>{intervalLeft}</strong><small>{paused ? 'Open full controls to resume' : 'Until your next switch'}</small></div><ProgressIndicators intervalProgress={intervalProgress} phaseProgress={phaseProgress} cycleLabel={cycleLabel} /><div className="simple-gel"><i /> GEL IN <b>{gelLeft}</b></div>{showMusic && <MusicPlayer label={trackLabel} isPlaying={musicPlaying} position={musicPosition} duration={musicDuration} onToggle={onToggleMusic} onSeek={onSeekMusic} compact />}</section>;
}
