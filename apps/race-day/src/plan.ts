...
export const elapsed = (race: TimingSession, currentTime: number): number =>
  Math.max(0, (race.pausedAt ?? currentTime) - race.anchor - race.pausedTotal);
...
export function cycleSummaryFor(
  plan: Phase[],
  race: TimingSession,
  currentTime: number,
  ...
  const phase = plan[race.phase] ?? plan[0];
  const cycleMs =
    Math.max(MINIMUM_CUE_DURATION_MS, phase.runDurationMs) + Math.max(MINIMUM_CUE_DURATION_MS, phase.walkDurationMs);
...
export function intervalBoundaryDelta(
  plan: Phase[],
  race: TimingSession,
  currentTime: number,
  direction: -1 | 1,
