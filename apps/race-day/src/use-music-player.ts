...
export function useMusicPlayer(tracks: readonly MusicTrack[], volume: number) {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (Date.now() - lastPersistedAt.current >= MUSIC_POSITION_PERSIST_MS) {
  ...
  const play = useCallback(
    (id: string) => {
      ...
      if (!audio) return;
      const restore = () => {
        if (trackIdRef.current === id) audio.currentTime = positionsRef.current[id] ?? 0;
      };
  ...
  const seek = useCallback((next: number) => {
    ...
    positionsRef.current[trackIdRef.current] = next;
    saveMusicPositions(positionsRef.current);
    setPosition(next);
  }, []);
